...
create table p_goods(
       ...
       g_num number(3) default 1,
       g_brief varchar2(100),
       foreign key(c_id) references p_category(c_id) on delete cascade        
);

--订单表,主键o_id,外键用户表中u_id,o_state表示商品订单状态,0表示未发货,1表示已发货,2表示已确认收货,默认0
