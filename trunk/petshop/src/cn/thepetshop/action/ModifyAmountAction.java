...
//		userid = "2";//¡Ÿ ±
		...
		String result = "";
		if(newnum.equals("0")){
			if (pd.delGoodsInCart(goodsid, userid)){
				result = "changesuccess";
			}else{
				result = "unchange";
			}
		}else{
			result = pd.modifyAmount(userid,goodsid,newnum);
		}
		PrintWriter out;
		try {
		...
		String goodsnumincart = pd.getNumInCart(userid);
		request.getSession().setAttribute("goodsnumincart", goodsnumincart);
		return null;
	}
}